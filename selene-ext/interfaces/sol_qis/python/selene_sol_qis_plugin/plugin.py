"""
The Sol QIS interface is split into two conceptual parts.

The first is a dynamic library (interface) comprising the shims from QIS
functionality (specific to Sol) to the Selene library functions, e.g.:
```
    ___rp(qubit, theta, phi) -> selene_rxy(selene_instance, qubit, theta, phi)
```
...
----------------        --------------------
            ...
            |                |
          -----------------------     -------------------------
          |   Dynamic sol-qis   |_____| Dynamic base QIS shim |
...
@dataclass
class SolQISShimInterface(BaseQISInterface):
    ...
    def library_file(self):
        # The Sol interface is now split into a static launcher and a shared
        # runtime. The planner links the launcher into the final executable and
        # adds the shared runtime separately.
        lib_name = "sol_qis_selene_interface_logging_"
...
class SolInterface(QuantumInterface):
    ...
    @property
    def dependencies(self):
        return [SolQISShimInterface(log_level=self.log_level)]
    ...
    def library_file(self):
        ...
        # runtime. The planner links the launcher into the final executable and
        # adds the shared runtime separately.
        lib_name = "sol_qis_selene_runner"
        lib_dir = Path(__file__).parent / "_dist/lib/"
        match platform.system():
            case "Linux":
